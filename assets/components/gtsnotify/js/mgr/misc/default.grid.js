gtsNotify.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper gtsnotify-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('gtsnotify-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('gtsnotify-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('gtsnotify-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('gtsnotify-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    gtsNotify.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(gtsNotify.grid.Default, MODx.grid.Grid, {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = gtsNotify.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('gtsnotify-grid-default', gtsNotify.grid.Default);